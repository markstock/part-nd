...
cell_ptr build_new_tree(fileprop_ptr,sim_ptr,cell_ptr);
 ...
 */
cell_ptr build_new_tree(fileprop_ptr file,sim_ptr sim,cell_ptr oldtop) {
   ...
   for (i=0;i<DIM;i++) {
      ...
      // NOTE the 0.7 factor---make the top level box too big
      // size = 0.7*(max[i]-min[i]);
...
int remake_box(sim_ptr sim,cell_ptr curr_cell,cell_ptr newtop) {
   ...
   // finally, free this box
   // if (curr_cell) free(curr_cell);
...
int find_particle_bounds(cell_ptr curr_cell,FLOAT* min,FLOAT* max) {
   ...
   if (curr_cell->has_subcells) {
      ...
      while (curr) {
         for (i=0;i<DIM;i++) {
            if (curr->x[0][i] < min[i]) min[i] = curr->x[0][i];
            if (curr->x[0][i] > max[i]) max[i] = curr->x[0][i];
         }
#ifdef MASS_IS_DIMENSION
         if (curr->logmass < min[DIM]) min[DIM] = curr->logmass;
         if (curr->logmass > max[DIM]) max[DIM] = curr->logmass;
#endif
         curr = curr->next;
      }
   }
...
int add_particle_to_cell(sim_ptr sim,particle_ptr curr,cell_ptr cell){
   ...
   if (cell->has_subcells) {
      ...
      i=0;
      for (d=0;d<DIM;d++) {
         if (curr->x[0][d] > cell->mid[d]) i+=pow(2,d);
      }
#ifdef MASS_IS_DIMENSION
      if (curr->logmass > cell->mid[DIM]) i+=pow(2,DIM);
#endif
      // fprintf(stdout,"  putting particle %d in cell %d\n",curr->index,i);
      add_particle_to_cell(sim,curr,cell->s[i]);
   } else {
      // otherwise the particle must go in this cell
      oldfirst = cell->first;
      curr->next = oldfirst;
      cell->first = curr;
      // cell->mass += curr->mass;	// do we do this later?
      cell->num++;
   }
...
int split_cell(sim_ptr sim,cell_ptr parent){
   ...
   while (curr) {
      ...
      i = 0;
      for (d=0;d<DIM;d++) {
         if (curr->x[0][d] > parent->mid[d]) i+=pow(2,d);
      }
#ifdef MASS_IS_DIMENSION
      if (curr->logmass > parent->mid[DIM]) i+=pow(2,DIM);
#endif
...
#ifdef MASS_IS_DIMENSION
         if (curr->logmass < curr_cell->min[DIM] || curr->logmass > curr_cell->max[DIM]) {
            out_of_bounds = TRUE;
            // return(-1);
         }
#endif
...
/*
            ...
            for (i=0;i<DIM;i++)
               if (moving_particle->x[0][i] < top->min[i] || moving_particle->x[0][i] > top->max[i]) {
                  // old way: remove the particle, continue
                  // out_of_bounds = TRUE;
                  // i+=DIM;
                  // new way: remake the trees with a larger size
                  return(-1);
               }
*/
...
int find_cell_cm(cell_ptr curr_cell,int do_level){
   ...
   if (curr_cell->has_subcells) {
      ...
      while (curr) {
         mtot += curr->mass;
         for (i=0;i<DIM;i++) com[i] += curr->x[0][i] * curr->mass;
         curr = curr->next;
      }
